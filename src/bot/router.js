// src/bot/router.js — ВИПРАВЛЕНА ВЕРСІЯ З ПІДТРИМКОЮ handleCallback/handleText

import { handleStart, handleCallback, handleText } from '../features/onboarding/handlers.js';
import { handleCallback as dashboardCallback, handleText as dashboardText } from '../features/dashboard/index.js';
...
import { handleCallback as affirmationsCallback } from '../features/affirmations/index.js';
import { handleCallback as reportsCallback } from '../features/reports/index.js';
import subscriptionCallbacks from '../features/subscription/index.js';
...
export const initRouter = (bot) => {
  ...
  bot.on('callback_query', async (ctx, next) => {
    ...
    try {
      ...
      if (await affirmationsCallback?.(ctx)) {
        try { await ctx.answerCbQuery(); } catch {}
        return;
      }

      if (await reportsCallback?.(ctx)) {
        try { await ctx.answerCbQuery(); } catch {}
        return;
      }

      // Замініки на subscription кнопки (якщо є)
      if (data?.startsWith('subscribe_') || data?.startsWith('renew_') || data === 'subscription_info') {
        try { await ctx.answerCbQuery(); } catch {}
        return;
      }
...
// ── Допоміжні функції для daily sessions ─────────────────────────────────────
