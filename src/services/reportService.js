...
const reportService = {
  async generateWeeklyReport(userTGId) {
    try {
      ...
      if (!records.length) return '❗ У тебе ще немає щоденних рефлексій за тиждень.';

      let report = '📊 Тижневий звіт твоїх рефлексій:\n\n';
      records.reverse().forEach((r, idx) => {
        report += `День ${idx + 1}:\n`;
        report += `🌟 Стан: ${r.fields.State || '-'}\n`;
        report += `⚡️ Енергія: +${r.fields['Energy Gain'] || '-'} / -${r.fields['Energy Loss'] || '-'}\n`;
  ...
  async generateMonthlyReport(userTGId) {
    try {
      ...
      if (!records.length) return '❗ У тебе ще немає щоденних рефлексій за місяць.';
