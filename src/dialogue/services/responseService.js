...
export const createOrUpdateResponse = async (
  ...
  try {
    ...
    if (existingRecords.length > 0) {
      ...
      await base('Responses').update([{ id: recordId, fields: fieldsToUpdate }]);
    } else {
      ...
      await base('Responses').create([{ fields: newRecordFields }]);
    }
  } catch (error) {
    throw error;
  }
...
export default { createOrUpdateResponse, isSessionCompleted, getDayRecord, getUserRecords };